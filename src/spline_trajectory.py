...
class SplineTrajectory:
    ...
    def update_spline(self):
        try:
            ...
            # Definir o ponto inicial como a posição atual do drone
            initial_point = np.array([self.x, self.y, self.z])
            if not np.array_equal(initial_point, self.targets[0]):
                self.targets.insert(0, initial_point)
            ...
            try:
                spline_x = CubicSpline(time, [t[0] for t in self.targets])
                spline_y = CubicSpline(time, [t[1] for t in self.targets])
                spline_z = CubicSpline(time, [t[2] for t in self.targets])
            except:
            ...
            try:
                self.spline_x = CubicSpline(time, [t[0] for t in self.targets])
                self.spline_y = CubicSpline(time, [t[1] for t in self.targets])
                self.spline_z = CubicSpline(time, [t[2] for t in self.targets])
            except:
                print("DUPLICATE TIMES")
                return
